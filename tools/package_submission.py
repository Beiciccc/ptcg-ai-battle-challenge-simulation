...
def main() -> int:
    ...
    parser.add_argument("--format", choices=["tar.gz", "zip"], default="tar.gz", help="Package format.")
    ...
    if args.format == "zip":
        ...
        output = SUBMISSIONS_DIR / f"{args.name}.tar.gz"
        package = build_submission_tar_gz(
            source,
            output,
            require_support_files=not args.allow_missing_cg,
        )
