...
def target_score(pokemon: Pokemon) -> int:
    ...
    if pokemon.id == 112 and len(pokemon.energies) >= 1:  # Munkidori
        score += 300
    if pokemon.id in {C.ALAKAZAM, C.OLD_ALAKAZAM}:
        score += 900
    elif pokemon.id == C.KADABRA:
        score += 650
    elif pokemon.id in {C.ABRA, C.OLD_ABRA}:
        score += 350
    if pokemon.id == C.HOPS_CRAMORANT:
        score += 850
    elif pokemon.id == C.HOPS_TREVENANT:
        score += 750
    elif pokemon.id == C.HOPS_PHANTUMP:
        score += 300
    elif pokemon.id == C.HOPS_SNORLAX:
        score += 250
    score += pokemon.hp
    return score
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    if self.state.firstPlayer != self.my_index and not self._opponent_is_crustle_wall():
                        if op_pokemon.id == C.RIOLU:
                            score += 250
                        elif op_pokemon.id == C.MEGA_LUCARIO_EX and len(op_pokemon.energies) >= 1:
                            score += 350
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id in {C.MAKUHITA, C.HARIYAMA}:
            ...
            if self._opponent_is_crustle_wall():
                ...
                score += 45 if energy_count < 3 else 0
            ...
            score += 150 if energy_count < 2 else 0
            score -= 20 if self.has_ready_lucario_line else 0
        return score
...
def _normalize_selection(selection, obs_dict) -> list[int]:
    ...
    if isinstance(selection, list):
        for idx in selection:
            if not isinstance(idx, int):
                continue
            if idx in seen or idx < 0 or idx >= n_options:
                continue
            cleaned.append(idx)
            seen.add(idx)
    ...
    target_count = min(min_count, limit)
