"""Diary Day 3 — IL rerank + Day2 heuristic fallback."""

"""Shared featurization for diary IL agent (stdlib only)."""
...
def featurize(obs_dict: dict, option_index: int) -> list[float]:
    ...
    otype = int(opt.get("type", 0))
    ctx = int(sel.get("context", 0))
    n_opt = max(1, len(opts))

    me_i = int(cur.get("yourIndex", 0))
    players = cur.get("players") or [{}, {}]
    me = players[me_i] if me_i < len(players) else {}
    op = players[1 - me_i] if (1 - me_i) < len(players) else {}

    feats = [
        1.0,
        otype / 16.0,
        ctx / 48.0,
        option_index / max(1, n_opt - 1),
        _num(sel.get("minCount")) / 10.0,
        _num(sel.get("maxCount")) / 10.0,
        len(me.get("prize") or []) / 6.0,
        len(op.get("prize") or []) / 6.0,
        _num(me.get("handCount")) / 20.0,
        _num(op.get("handCount")) / 20.0,
        _num(me.get("deckCount")) / 60.0,
        _num(cur.get("turn")) / 100.0,
        1.0 if otype == 7 else 0.0,
        1.0 if otype == 13 else 0.0,
        1.0 if otype == 8 else 0.0,
        1.0 if otype == 9 else 0.0,
        1.0 if otype == 14 else 0.0,
        1.0 if otype == 3 else 0.0,
        _num(opt.get("attackId")) / 1200.0,
        _num(opt.get("index")) / 30.0,
        _num(opt.get("playerIndex")) / 2.0,
        1.0 if int(opt.get("inPlayArea", -1)) == 4 else 0.0,
        1.0 if int(opt.get("inPlayArea", -1)) == 5 else 0.0,
        _num(opt.get("number")) / 10.0,
        1.0 if cur.get("energyAttached") else 0.0,
        1.0 if cur.get("supporterPlayed") else 0.0,
        1.0 if int(cur.get("firstPlayer", -1)) == me_i else 0.0,
        _num(me.get("active") and len(me["active"])) / 1.0,
        _num(op.get("active") and len(op["active"])) / 1.0,
        _num(me.get("bench") and len(me["bench"])) / 8.0,
        _num(op.get("bench") and len(op["bench"])) / 8.0,
        1.0 if otype == 1 else 0.0,
    ]
...
def score_options(obs_dict: dict, weights: list[float], bias: float) -> list[float]:
    sel = obs_dict.get("select") or {}
    opts = sel.get("option") or []
    scores = []
    for i in range(len(opts)):
        x = featurize(obs_dict, i)
        s = bias + sum(weights[j] * x[j] for j in range(min(len(weights), len(x))))
        scores.append(s)
    return scores
...
# Setup-bench / discard / damage-counter scoring. The gold rival agent does NOT
# score these (engine default), and in the mirror it's the ONLY thing that
# differs from this agent -- yet it wins ~70-30. So this is defaulted OFF as the
# suspected regression. A/B it vs that agent (head_to_head) and set True only if
# ON measurably wins.
...
POLICY_W = [1.2000111, -4.5680197, -1.534504, -1.3286543, 2.6788905, 1.6991113, -0.069490244, -0.36717409, -1.2699095, 0.37818647, 0.22186715, 0.47581796, -1.5191134, 0.025882614, -1.5878992, -0.81422593, -0.65128334, -1.9856148, -0.056416996, -1.2184044, 0.15938925, -0.94871619, -1.4534089, 0.33168174, 0.18329677, 0.063495298, 0.069542914, -0.18122757, 0.84211983, -0.27819954, -0.079728363, 1.4338349]
POLICY_B = 2.9755692741086874
IL_TOPK = 6


def il_rerank(obs_dict, ranked):
    """Rerank only among the heuristic top-k (safe distillation)."""
    if not ranked:
        return ranked
    k = min(IL_TOPK, len(ranked))
    head = ranked[:k]
    tail = ranked[k:]
    head = sorted(head, key=lambda i: score_options(obs_dict, POLICY_W, POLICY_B)[i], reverse=True)
    return head + tail


def agent(obs_dict):
    ranked = submit_heuristic(obs_dict)
    if obs_dict.get("select") is None:
        return ranked
    try:
        return il_rerank(obs_dict, ranked)
    except Exception:
        return ranked
