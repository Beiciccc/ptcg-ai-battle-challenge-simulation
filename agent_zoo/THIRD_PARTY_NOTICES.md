# Third-party Notices

Some strategy snapshots reproduce source published through public Kaggle Code.
Kaggle describes the public notebook code corpus as Apache-2.0 licensed. The
corresponding upstream notebook URL, when recorded in the experiment history,
is preserved in `ORIGIN.json` and the Agent Zoo manifest.

Snapshot files are preserved byte-for-byte and are not relicensed by this
repository's MIT license. Attribution and upstream terms remain attached to
each source. An unversioned upstream URL is explicitly marked as unpinned; it
must not be treated as proof that the current notebook version produced the
historical bytes.

Deck lists and strategy source do not include the Kaggle competition runtime,
engine binaries, model weights, raw competition data, or replay data. Obtain
the public runtime directly from Kaggle after accepting the applicable rules.

Apache License 2.0 text: `LICENSES/Apache-2.0.txt`.
